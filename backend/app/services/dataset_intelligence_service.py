...
IDENTIFIER_PATTERNS = re.compile(
    r"^(id|.*_id|.*id|code|key|uuid|ref|reference|account.*num)", re.I
)
DATE_PATTERNS = re.compile(
    r"^(date|time|timestamp|year|month|day|quarter|period|created_at|updated_at)", re.I
)
GEO_PATTERNS = re.compile(
    r"^(country|region|city|state|address|zip|postal|latitude|longitude|location)", re.I
)
REVENUE_PATTERNS = re.compile(
    r"^(revenue|sales|income|turnover|gross|total.*amount)", re.I
)
COST_PATTERNS = re.compile(
    r"^(cost|expense|spend|cogs|overhead|operating.*cost)", re.I
)
PROFIT_PATTERNS = re.compile(
    r"^(profit|margin|net.*income|ebitda|earnings)", re.I
)
KPI_PATTERNS_R2 = re.compile(
    r"^(score|rate|ratio|percent|index|count|amount|value|total|balance|growth|roi|kpi)", re.I
)
TARGET_PATTERNS = {
...
def classify_column(name: str, dtype: str, nunique: int, nrows: int,
    ...
    if dtype in ("int64", "float64", "int32", "float32", "int", "float"):
        ...
        if REVENUE_PATTERNS.match(col_lower):
            result["classification"] = "kpi"
            result["kpi_type"] = "revenue"
        elif COST_PATTERNS.match(col_lower):
            result["classification"] = "kpi"
            result["kpi_type"] = "cost"
        elif PROFIT_PATTERNS.match(col_lower):
            result["classification"] = "kpi"
            result["kpi_type"] = "profit"
        elif KPI_PATTERNS_R2.match(col_lower):
            result["classification"] = "kpi"
            result["kpi_type"] = "metric"
        elif nunique <= 10:
            result["classification"] = "categorical"
            result["cardinality"] = "low"
        else:
            result["classification"] = "numeric"
            result["cardinality"] = "high" if nunique > 100 else "low"

        # Check if potentially a target variable
        ...
        if nunique == nrows and nrows > 100:
            ...
            result["classification"] = "categorical"
            result["cardinality"] = "medium"
        else:
            result["classification"] = "text"
            result["cardinality"] = "high"
...
def detect_business_context(df: pd.DataFrame) -> str:
    """Detect the business domain from column names."""
...
def detect_target_variable(columns: list[dict[str, Any]], df: pd.DataFrame) -> dict[str, Any]:
    """Detect the most likely target variable for prediction."""
    # First pass: check for explicit target patterns
...
def analyze_dataset(df: pd.DataFrame) -> dict[str, Any]:
    """Run full Dataset Intelligence Engine analysis."""
    ...
    for col in df.columns:
        ...
        info = classify_column(col, dtype, nunique, nrows, sv)
        columns.append(info)

    dataset_type = detect_business_context(df)
