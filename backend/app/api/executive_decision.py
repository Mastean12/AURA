...
async def executive_decision_intelligence(payload: DecisionRequest):
    """Generate executive-level decision intelligence from model outputs."""
    import io
    ...
    explainability_result = await run_explainability(
    ...
    scenario_result = await run_scenario_analysis(payload.doc_id)

    result = await run_executive_decision_intelligence(
        automl_result, explainability_result, scenario_result
    )
